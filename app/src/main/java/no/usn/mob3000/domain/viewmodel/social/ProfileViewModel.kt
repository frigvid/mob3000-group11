package no.usn.mob3000.domain.viewmodel.social
...
import no.usn.mob3000.domain.usecase.social.FetchUserByIdUseCase
import no.usn.mob3000.domain.usecase.social.FetchUserProfileUseCase
import no.usn.mob3000.domain.usecase.social.FriendRequestUseCase
import no.usn.mob3000.domain.usecase.social.GetUserGameStatsUseCase
import no.usn.mob3000.domain.usecase.social.UpdateProfileUseCase

/**
 * ViewModel for handling user profile-related functionality.
 *
 * Using the different use cases to communicate with the business logic handled in the data layer
 *
...
class ProfileViewModel(
    ...
    private val _userGameStats = MutableStateFlow(Result.success(UserGameStats(0, 0, 0)))
    ...
    /**
     * Initializes the ViewModel by fetching the current user's ID and fetching their profile.
     * Used primary to ensure better refreshes, as this part of the system will not
     ...
     *
     * @param friendRequestId The friend request's UUID.
     */
    fun acceptFriendRequest(friendRequestId: String) {
        viewModelScope.launch {
            ...
            result.onSuccess {
                ...
                Logger.e("Error when accepting: ${it.message}")
            }
        }
    }
    ...
    fun declineFriendRequest(friendRequestId: String) {
        viewModelScope.launch {
            ...
            result.onSuccess {
                ...
                Logger.e("Error when declining: ${it.message}")
            }
        }
    }
    ...
    fun fetchUser(userId: String) {
        viewModelScope.launch {
            ...
            result.onSuccess { userProfile ->
                ...
                Logger.e("Failed to fetch user profile: ${it.message}")
            }
        }
    }
    ...
    fun fetchUserGameStats() {
        viewModelScope.launch {
            ...
            result.onSuccess { _ ->
                ...
                Logger.e("Failed to fetch user game stats: ${it.message}")
            }
        }
    }
    ...
    fun fetchUserById(userId: String) {
        viewModelScope.launch {
            ...
            result.onSuccess { userProfile ->
                userProfile?.let {
                    _userProfiles.value += (userId to it)
                }
            }
        }
    }
     ...
     *
     * @param selectedUser The selected user's [UserProfile].
     */
